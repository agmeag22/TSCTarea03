...
float calculateLocalD(int i,mesh m){
	...
	row1.push_back(calcularTenedor(e,EQUIS,2,1,m)); row1.push_back(calcularTenedor(e,YE,2,1,m)); row1.push_back(calcularTenedor(e,ZETA,2,1,m));
	row2.push_back(calcularTenedor(e,EQUIS,3,1,m)); row2.push_back(calcularTenedor(e,YE,3,1,m)); row2.push_back(calcularTenedor(e,ZETA,3,1,m));
	row3.push_back(calcularTenedor(e,EQUIS,4,1,m)); row2.push_back(calcularTenedor(e,YE,4,1,m)); row2.push_back(calcularTenedor(e,ZETA,4,1,m));
...
float calculateLocalArea(int i,mesh m){
    ...
    D = (a * k - c * g) * w - a * f * h - b * (d * k - f * g) + c * d * h;
...
void calculateLocalA(int i,Matrix &A,mesh m){
    ...
    node n2 = m.getNode(e.getNode2()-1);
    node n3 = m.getNode(e.getNode3()-1);
    node n4 = m.getNode(e.getNode3()-1);

    A.at(0).at(0) = calcularTenedor(e,YE,3,1,m)*calcularTenedor(e,ZETA,4,1,m)-calcularTenedor(e,YE,4,1,m)*calcularTenedor(e,ZETA,3,1,m);
    A.at(0).at(1) = calcularTenedor(e,YE,4,1,m)*calcularTenedor(e,ZETA,2,1,m)-calcularTenedor(e,YE,2,1,m)*calcularTenedor(e,ZETA,4,1,m);
    A.at(0).at(2) = calcularTenedor(e,YE,2,1,m)*calcularTenedor(e,ZETA,3,1,m)-calcularTenedor(e,YE,3,1,m)*calcularTenedor(e,ZETA,2,1,m);

    A.at(1).at(0) = calcularTenedor(e,EQUIS,4,1,m)*calcularTenedor(e,ZETA,3,1,m)-calcularTenedor(e,EQUIS,3,1,m)*calcularTenedor(e,ZETA,4,1,m);
    A.at(1).at(1) = calcularTenedor(e,EQUIS,2,1,m)*calcularTenedor(e,ZETA,4,1,m)-calcularTenedor(e,EQUIS,4,1,m)*calcularTenedor(e,ZETA,2,1,m);
    A.at(1).at(2) = calcularTenedor(e,EQUIS,3,1,m)*calcularTenedor(e,ZETA,2,1,m)-calcularTenedor(e,EQUIS,2,1,m)*calcularTenedor(e,ZETA,3,1,m);

    A.at(2).at(0) = calcularTenedor(e,EQUIS,3,1,m)*calcularTenedor(e,YE,4,1,m)-calcularTenedor(e,EQUIS,3,1,m)*calcularTenedor(e,YE,4,1,m);
    A.at(2).at(1) = calcularTenedor(e,EQUIS,4,1,m)*calcularTenedor(e,YE,2,1,m)-calcularTenedor(e,EQUIS,2,1,m)*calcularTenedor(e,YE,4,1,m);
    A.at(2).at(2) = calcularTenedor(e,EQUIS,2,1,m)*calcularTenedor(e,YE,3,1,m)-calcularTenedor(e,EQUIS,3,1,m)*calcularTenedor(e,YE,2,1,m);
}
//CAMBIADO
void calculateB(Matrix &B){
    ...
    B.at(2).at(0) = -1; B.at(2).at(1) = 0; B.at(2).at(2) = 0; B.at(2).at(3) = 1; B.at(2).at(4) = -1; B.at(2).at(5) = 0;B.at(2).at(6) = 0;B.at(2).at(7) = 1;B.at(2).at(8) = -1;B.at(2).at(9) = 0;B.at(2).at(10) = 0;B.at(2).at(11) = 1;
}
//CAMBIADO
...
Matrix createLocalK(int e,mesh &m){
    ...
    calculateB(B);
    productRealMatrix(u_bar*J/D,productMatrixMatrix(e_star,productMatrixMatrix(A,B,3,3,12),3,3,12),alpha);
    ...
    transpose(A,At);
    transpose(B,Bt);
    productRealMatrix(nu*Ae/(D*D),productMatrixMatrix(Bt,productMatrixMatrix(At,productMatrixMatrix(A,B,3,3,12),3,3,12),12,3,12),beta);
...
void assemblyK(element e,Matrix localK,Matrix &K,int nnodes){
 int index1 = e.getNode1() - 1;
    ...
    int index5 = index1+nnodes, index6 = index2+nnodes, index7 = index3+nnodes, index8 = index4+nnodes;
    int index9 = index1+2*nnodes, index10 = index2+2*nnodes, index11 = index3+2*nnodes,index12 = index4+2*nnodes ;
    int index13 = index1+3*nnodes, index14 = index2+3*nnodes, index15 = index3+3*nnodes,index16 = index4+3*nnodes ;
